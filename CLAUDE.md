...
| Convention "toute règle de jeu validée côté serveur" (voir Conventions de code) initialement pas totalement respectée à la clôture du Sprint 2 : le franchissement et la destruction d'obstacle passaient par des routes serveur (`POST /cartes/:id/franchir` et `/detruire`), mais l'attaque du banc d'essai (`attaquerEnnemi`) restait calculée et appliquée uniquement côté client | **Résolu** : route `POST /cartes/:id/attaquer` ajoutée (`carte.routes.ts`), qui appelle les fonctions pures `determinerModeAttaque`/`determinerPorteeAttaquePersonnage`/`resoudreAttaque` (`combat-resolution.service.ts`, testées) pour déterminer mode/portée et appliquer les dégâts fixes. `combat-test.ts` appelle cette route au lieu de calculer l'issue localement (PV de l'ennemi toujours fournis par le client, aucune table `Creature` persistée avant le Sprint 3) |
