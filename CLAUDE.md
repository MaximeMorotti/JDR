...
- **Un seul compagnon par équipe**, jamais plus, choisi après la création de tous les personnages. Accessible même en équipe solo (1 personnage). Certains compagnons ont un prérequis : Élan → **classe** Chasseur sylvestre, Gnome → **classe** Mage, Sanglier → **classe** Ingénieur dans l'équipe — mais Fée → **race** Demi-Orc (pas une classe : "Demi-Orc" n'est jamais une classe, voir la matrice classes × races). Modélisé séparément en base (`CompagnonRef.raceRequiseId` pour la Fée, `CompagnonClasseLiee` pour les 3 autres).
