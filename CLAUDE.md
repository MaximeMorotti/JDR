...
- [x] **Sprint 2 — Déplacement sur carte de combat (TERMINÉ)** : grille octogonale à 8 directions, obstacle générique avec presets de PV (Léger/Moyen/Lourd) et case « infranchissable de zone », franchissement et destruction au jet de dé (d20 + Dextérité/Force, table d'interaction raciale des 5 races, esquive Elfe sur les obstacles légers), case Tranchée (jamais bloquante, coût doublé pour les non-Nains), attaque de base sur un ennemi dans le banc d'essai (mêlée/distance selon la catégorie d'arme équipée, dégâts fixes), carte de combat persistée en base (`Carte`, seedée) et chargée par `combat-test.ts` via l'API au lieu de données codées en dur, moteur de résolution pur côté serveur (`combat-resolution.service.ts`) couvert par la première suite de tests du projet (vitest, 47 tests) — voir écart ci-dessous : l'attaque reste calculée côté client, sans route serveur dédiée
...
| Convention "toute règle de jeu validée côté serveur" (voir Conventions de code) pas totalement respectée à la clôture du Sprint 2 : le franchissement et la destruction d'obstacle passent bien par des routes serveur (`POST /cartes/:id/franchir` et `/detruire`, `carte.routes.ts`) qui appellent le moteur de résolution pur, mais l'attaque du banc d'essai (`attaquerEnnemi`) reste calculée et appliquée uniquement côté client dans `combat-test.ts` | Écart assumé et documenté en commentaire dans `combat-resolution.service.ts` : les fonctions pures `determinerModeAttaque`/`determinerPorteeAttaquePersonnage`/`determinerPorteeAttaqueCreature` existent et sont testées côté serveur, mais aucune route ne les expose encore. À corriger avant/pendant le Sprint 3, quand le vrai moteur de combat remplace de toute façon ce placeholder (dégâts fixes, portée d'attaque joueur = portée de déplacement) |
